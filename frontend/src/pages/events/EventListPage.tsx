...
export const EventListPage = () => {
  ...
  const registerMutation = useMutation({
    ...
    onError: (err: any) => {
      toast.error(err.response?.data?.errors?.event_id?.[0] || 'Gagal mendaftar event.');
    },
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Event Donor Darah</h1>
          <p className="text-sm text-gray-500 mt-1">Jadwal kegiatan donor darah PMI terdekat</p>
        </div>
        <button
          onClick={() => setUpcomingOnly(!upcomingOnly)}
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium border transition-colors ${
            upcomingOnly ? 'bg-red-600 text-white border-red-600' : 'border-gray-200 text-gray-600 hover:bg-gray-50'
          }`}
        >
          <Filter size={14} />
      ...
      {events.length === 0 ? (
        <EmptyState
          ...
          description={upcomingOnly ? 'Belum ada event donor darah yang akan datang.' : 'Belum ada event donor darah.'}
        />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {events.map((event) => (
            <div key={event.id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 card-hover flex flex-col">
              <div className="flex items-start justify-between gap-3 mb-4">
                <div className="w-12 h-12 bg-red-50 rounded-xl flex items-center justify-center flex-shrink-0">
                  <MapPin className="text-red-600" size={20} />
                </div>
                <div className="flex-1">
                  <h3 className="font-bold text-gray-900 text-sm mb-1 line-clamp-2">{event.name}</h3>
              ...
              <p className="text-xs text-gray-500 mb-4 line-clamp-2 flex-1">{event.description}</p>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={event.registered_count >= event.quota || registerMutation.isPending}
                ...
                >
                  {event.registered_count >= event.quota ? 'Penuh' : 'Daftar'}
                </button>
              </div>
            </div>
          ))}
