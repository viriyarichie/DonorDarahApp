...
export const KelolaStokPage = () => {
  ...
  const [form, setForm] = useState({ blood_type: '', amount: '', location_id: '' });
  ...
  const createMutation = useMutation({
    ...
    onError: (err: any) => toast.error(err.response?.data?.message || 'Gagal menambahkan stok.'),
  });

  const updateMutation = useMutation({
    mutationFn: () => stockService.update(editTarget!.id, {
      blood_type: form.blood_type,
      amount: Number(form.amount),
      location_id: Number(form.location_id),
    }),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Kelola Stok Darah</h1>
          <p className="text-sm text-gray-500 mt-1">Manajemen ketersediaan stok darah di setiap lokasi</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 animate-fade-in">
          <h3 className="font-bold text-gray-900 mb-4">{editTarget ? 'Edit Stok' : 'Tambah Stok Baru'}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Jumlah (kantong) *</label>
          ...
          <div className="flex gap-3 mt-4">
            ...
            <button
              ...
              disabled={!form.blood_type || !form.amount || !form.location_id || createMutation.isPending || updateMutation.isPending}
              className="flex-1 py-2.5 pmi-gradient text-white rounded-xl text-sm font-medium hover:opacity-90 disabled:opacity-60">
              {createMutation.isPending || updateMutation.isPending ? 'Menyimpan...' : editTarget ? 'Perbarui Stok' : 'Simpan Stok'}
            </button>
          </div>
        </div>
      )}
      ...
      {stocks.length === 0 ? (
        <EmptyState icon={Droplets} title="Belum ada data stok" description="Tambahkan data stok darah untuk setiap lokasi PMI." />
      ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  ...
                  <th className="text-left px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Terakhir Update</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {stocks.map(stock => (
                  <tr key={stock.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4"><BloodTypeBadge type={stock.blood_type} /></td>
                    <td className="px-6 py-4">
                      <div className="text-sm font-medium text-gray-800">{stock.location?.name}</div>
                      <div className="text-xs text-gray-400 capitalize">{stock.location?.type?.replace('_', ' ')}</div>
                    </td>
                    <td className="px-6 py-4">
