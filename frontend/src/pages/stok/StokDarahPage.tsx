      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Informasi ketersediaan darah di seluruh lokasi PMI
          </p>
        </div>
        <button
          onClick={() => refetch()}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {Object.entries(summary).map(([type, info]) => {
            ...
            return (
              <div
                key={type}
                className={`${cfg.bg} ${cfg.border} border rounded-2xl p-5`}
              >
                ...
                <div className={`text-xs font-medium ${cfg.text} mt-2`}>
                  {cfg.label}
                </div>
              </div>
            );
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="flex items-center gap-2 flex-1">
          ...
          <select
            ...
            className="flex-1 px-3 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-red-400 text-black"
          >
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-50">
                {stocks.map((stock) => {
                  ...
                  const cfg =
                    statusConfig[color as keyof typeof statusConfig] ||
                    statusConfig.green;
                  return (
                    <tr
                      key={stock.id}
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${cfg.bg} ${cfg.border} ${cfg.text}`}
                        >
                          {cfg.label}
                        </span>
                      </td>
