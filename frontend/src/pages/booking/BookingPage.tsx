...
import { Calendar, MapPin, CheckCircle, Clock, XCircle, Plus } from 'lucide-react';
...
export const BookingPage = () => {
  ...
  const [form, setForm] = useState({ location_id: '', booking_date: '', notes: '' });
  ...
  const createMutation = useMutation({
    ...
    onError: (err: any) => {
      const msg = err.response?.data?.errors?.booking_date?.[0] || err.response?.data?.message || 'Gagal membuat booking.';
      toast.error(msg);
    },
  });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Buat dan pantau jadwal donor darah Anda</p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 animate-fade-in">
          <h3 className="font-bold text-gray-900 mb-4">Buat Jadwal Donor Baru</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Pilih Lokasi *</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Tanggal Donor *</label>
              <input
                type="date"
            ...
            <div className="sm:col-span-2">
              ...
              <textarea
                ...
                className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-red-400 resize-none"
              />
            </div>
          </div>
          <div className="flex gap-3 mt-4">
            ...
            <button
              ...
              disabled={!form.location_id || !form.booking_date || createMutation.isPending}
      ...
      {bookings.length === 0 ? (
        ...
        <div className="space-y-3">
          {bookings.map((booking) => (
            <div key={booking.id} className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 card-hover">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="text-sm font-semibold text-gray-800">{booking.location?.name}</div>
                    <div className="text-xs text-gray-500 mt-0.5">{booking.location?.address}</div>
                    <div className="flex items-center gap-2 mt-2">
                      <Clock size={13} className="text-gray-400" />
                      <span className="text-xs text-gray-500">{booking.booking_date_formatted}</span>
