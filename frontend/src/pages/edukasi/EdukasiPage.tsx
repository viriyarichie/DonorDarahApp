...
export const EdukasiPage = () => {
  ...
  const stripHtml = (html: string) => html.replace(/<[^>]*>/g, '').slice(0, 150) + '...';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Artikel dan informasi seputar donor darah</p>
      </div>
      ...
      <div className="flex gap-3">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-red-400"
          />
        </div>
        <button
      ...
      {articles.length === 0 ? (
        <EmptyState
          ...
          description={search ? `Tidak ada artikel untuk pencarian "${search}"` : 'Belum ada artikel yang dipublikasikan.'}
        />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {articles.map((article) => (
            ...
            >
              ...
              <p className="text-sm text-gray-500 mb-3 line-clamp-3">{stripHtml(article.content)}</p>
