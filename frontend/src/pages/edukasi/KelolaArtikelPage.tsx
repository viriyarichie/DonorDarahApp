      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Buat dan kelola konten edukasi donor darah
          </p>
        </div>
        <button
          onClick={() => {
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-red-400 focus:ring-2 focus:ring-red-100 text-black"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">
              ...
              <textarea
                ...
                className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm outline-none focus:border-red-400 resize-y font-mono bg-gray-50 text-black"
              />
            </div>
            <label className="flex items-center gap-3 cursor-pointer group">
              <div
                className={`relative w-10 h-5 rounded-full transition-colors ${form.publish ? "bg-red-600" : "bg-gray-300"}`}
              >
                <div
                  className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${form.publish ? "translate-x-5" : "translate-x-0.5"}`}
                />
                <input
                  type="checkbox"
          ...
          {articles.map((article) => (
            ...
            >
              <div className="flex items-start gap-3 flex-1 min-w-0">
                <div
                  className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${article.is_published ? "bg-green-500" : "bg-gray-300"}`}
                />
                  ...
                  <div className="text-xs text-gray-400 mt-0.5 flex items-center gap-2">
                    <span>
                      {article.is_published
                        ? `✓ Dipublikasikan ${article.published_at}`
                        : "◦ Draft"}
                    </span>
                    <span>·</span>
